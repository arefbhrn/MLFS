...
quietly set lstCells [lsearch -all -inline $lstObjects "*\(fdce\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(FDCE\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(fdpe\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(FDPE\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(fdre\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(FDRE\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(fdse\)*"];
quietly lappend lstCells {*}[lsearch -all -inline $lstObjects "*\(FDSE\)*"];
...
for {set i 0} {$i < $faultInjectionIterations} {incr i} {
    for {set j 0} {$j < 100} {incr j} {
        ...
        if { [string match -nocase "\{*" "$cellName"] } {
            quietly set cellSignal "$cellName/q_out\}";
        } else {
            quietly set cellSignal "\{$cellName/q_out\}";
        }
        echo "    FF   :   $cellSignal";
        ...
        quietly set value [changeRandomBit $originalSignalValue 0 1];
