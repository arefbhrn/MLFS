...
for {set i 0} {$i < $faultInjectionIterations} {incr i} {
    ...
    for {set j 0} {$j < 100} {incr j} {
        ...
        if { [string match -nocase "\{*" "$cellName"] } {
            quietly set cellSignal "$cellName/DOBDO";
        } else {
            quietly set cellSignal "\{$cellName/DOBDO";
        }
        echo "    RAMB :   $cellSignal\}";
        quietly set originalSignalValue [examine "$cellSignal\}"];
        quietly set sigLen [string length $originalSignalValue];
        quietly set bitFaults 1;
        if { $bitFaults > $sigLen } {
            quietly set bitFaults $sigLen;
        }
        quietly set bitIndices "";
        while { [llength $bitIndices] < $bitFaults } {
            quietly set index [expr round(floor(rand() * $sigLen))]
            if { [lsearch $bitIndices $index] > -1 } {
                continue;
            }
            lappend bitIndices $index;
        }
        echo "    Bits :   \[$bitIndices\]";
        ...
        if { [catch {
            ...
            quietly set vals "";
            for {set k 0} {$k < [llength $bitIndices]} {incr k} {
                quietly set val [expr round(rand())];
                lappend vals $val
                force -freeze "${cellSignal}([lindex $bitIndices $k])\}" 'b$val $faultStartTime$timeUnit -cancel $faultEndTime$timeUnit;
                # force -freeze "${cellSignal}([lindex $bitIndices $k])\}" 'b1 $faultStartTime$timeUnit
            }
            echo "    Val1 :   \[$vals\]";
